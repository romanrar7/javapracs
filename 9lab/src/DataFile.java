...
public class DataFile {
    ...
    private static Clothes parse(String line) {
        ...
        if (p.length < 8) {
            System.out.println("Пропуск рядка: " + line);
            return null;
        }
        try {
            if ("PANTS".equals(p[0])) {
            ...
            if ("JACKET".equals(p[0])) {
                boolean wp = "так".equalsIgnoreCase(p[6]);
                return new Jacket(p[1], p[2], Double.parseDouble(p[3]), p[4], p[5], SizeCategory.M, wp, p[7]);
            }
            if ("SKIRT".equals(p[0])) {
            ...
            System.out.println("Помилка рядка: " + e.getMessage());
        }
        return null;
    }
